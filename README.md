...
由于本项目基于纯原生 Web 标准构建，**无需安装任何重型编译环境与复杂依赖**，以下提供最常用的两种运行方式：

### 方式一：直接双击打开（最简单 · 免安装）
克隆或下载本仓库后，**直接在电脑上双击 `index.html`**，即可直接在任意现代浏览器（Chrome / Edge / Safari / Firefox 等）中秒开体验全部 11 大模块！

---

### 方式二：使用本地轻量服务器启动（推荐 · 体验最佳）
...
2. **太乙八十一宫阵图 (81 Matrix)**：
   ...
   - 每一个单元格均带有权威角标位号 `(宫_位)`，如 `31 (四₄)`、`16 (七₂)`、`1 (一₁)`。
3. **三向精准动态联动与弹性震荡微交互**：
...
| **05** | [`wuxing.html`](wuxing.html) | ☯ **五行三元生克馆** | 水一火二木三金四土五原图生成数，相生相克 3D 几何流光闭环与三元映射。 |
